# Adapted from Simon Candelaresi's RK6 method implemented in the PENCIL-code
#
#
#
#
...
# With much of the code blatantly taken with permission from
...
def Dipole(xx, m = np.array([0,0,1])): # add check if xx < machine precision?
    mn = m/np.sqrt(np.sum(m**2)) #mnormalized
    xxn = xx/np.sqrt(np.sum(xx**2)) #xxnormalized Careful around Zero!
    vf = (3*xxn * np.dot(xxn, m) - m)/xxn # Needs the delta? Numerically really?
    return vf # add the delta if you dare!
...
def BHopf(xx, w1=1, w2=np.sqrt(2), r0=1, translation=np.array([0,0,0])):
        """
        ...
        Call signature:
           ...
           BHopf(xx, w1=1, w1=2, r0=1, translation=np.array([0,0,0])):
        ...
        *translation*
            Translates the entire field so the origin is at this x,y,z- coordinate

        *rotation*
            rotates such that the z-axis aligns with this vector (still to be implemented)
...
class stream:
    ...
    def __init__(self,xx, vvfn, integration = 'RK6', intdir= 'forward', hMin = 2e-6, hMax = .5, lMax = 10000, tol = 1e-4, iterMax = 100000, **kwargs ):
        ...
        call signature:
          ...
          streamInit(vvfn, xx, integration = 'RK6', hMin = 2e-6, hMax = .5, lMax = 10000, tol = 1e-4, iterMax = 100000)
